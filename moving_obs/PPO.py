...
class PPO:
    def __init__(self, state_dim, action_dim, lr_actor, lr_critic, gamma, K_epochs, eps_clip, has_continuous_action_space, action_std_init=0.6):
        ...
        dynamics_model = DynamicsNet().to(device)
    ...
    def find_reachability_grid(self):
        ...
        while reach_continue:
            ...
            with torch.no_grad():
                ...
                self.obs_curr_step = self.get_obstacles(self.verify_steps)
                reachability_loss = self.reachability_loss(lb, ub, self.obs_curr_step.repeat(lb.shape[0], 1, 1))
    ...
    def update(self):
        ...
        for _ in range(self.K_epochs):
            ...
            if self.use_reachability_loss and len(self.buffer_input) != 0:
                ...
                reachability_loss_prev = self.reachability_loss(lb_buffer, ub_buffer, self.buffer_obstacle)
                self.bound = 1000.0 
            ...
            if self.use_reachability_loss and input_selected is not None:
                ...
                self.bound = 1000.0 
                reachability_loss = self.reachability_loss(lb_selected, ub_selected, self.obs_curr_step.repeat(lb_selected.shape[0], 1, 1))
    ...
    def reachability_loss(self, lb_selected, ub_selected, obstacles):
        x_lb = lb_selected[:, 0]
        x_ub = ub_selected[:, 0]
        y_lb = lb_selected[:, 1]
        y_ub = ub_selected[:, 1]
        ...
        reachability_loss = loss1 + loss2 + loss3 + loss4 + loss5 + loss_obj_others
